...
!!! info
    The style guide is always a work in progress, and not all QuantumESPRESSOParser code
    follows the rules. When modifying QuantumESPRESSOParser, please fix the style violations
...
QuantumESPRESSOParser uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/QuantumESPRESSOParser.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the QuantumESPRESSOParser directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to QuantumESPRESSOParser have
